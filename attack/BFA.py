...
class BFA(object):
    ...
    def flip_bit(self, m,name,layer_map):
        '''
        ...
        w_id=np.asanyarray(w_idx_topk.to('cpu'))
        g=np.asanyarray(m.weight.data.to('cpu'))
        #查找梯度最大的卷积核
        original_indices = np.unravel_index(w_id, g.shape)

        layer_map[name]=original_indices[0]
...
# mask, then perform the bitwise xor operation to realize the bit-flip.
    ...
    def flip_bit2(self, m, name, layer_map):
        ...
        w_id = np.asanyarray(w_idx_topk.to('cpu'))
        g = np.asanyarray(m.weight.data.to('cpu'))
        # search the conv kernel with the largest gradient
        original_indices = np.unravel_index(w_id, g.shape)
        layer_map[name] = original_indices[0]
        ...
        bit_positions = torch.arange(m.N_bits, device=b_grad_topk.device).unsqueeze(1).repeat(1, self.k_top)
        
        valid_mask = (b_grad_topk != 0).float()
        
        priority = (m.N_bits - bit_positions) * 1000.0 + b_grad_topk.abs()
        priority *= valid_mask 

        _, b_grad_max_idx = priority.view(-1).topk(self.n_bits2flip)
        bit2flip = b_grad_topk.clone().view(-1).zero_()

        if torch.any(priority > 0):  
            bit2flip[b_grad_max_idx] = 1
            bit2flip = bit2flip.view(b_grad_topk.size())
        else:
            pass
        ...
        param_flipped2 = bin2int(w_bin, m.N_bits).view(m.weight.data.size()).float()

        return param_flipped2


    def progressive_bit_search(self, model, data, target,layer_list,layer_map):
        ''' 
        ...
        while self.loss_max <= self.loss.item():
            ...
            for name, module in model.named_modules():
                if isinstance(module, quan_Conv2d) or isinstance(
                    ...
                    attack_weight = self.flip_bit2(module,name,layer_map)
        ...
        print(f"the max_loss_module is{max_loss_module}")
        layer_list.append(max_loss_module)
